...
<div class="flex flex-col h-full overflow-hidden bg-surface-900 text-surface-200 select-none">
	...
	<div class="flex items-center gap-2 px-3 py-1 border-b border-surface-700 bg-surface-800/80 text-[11px] shrink-0">
		<span class="text-hecate-400 uppercase tracking-wider text-[10px]">LLM</span>
		{#if $selectedModel}
			<span class="text-surface-600">·</span>
			<span class="text-[10px] text-surface-300">{$selectedModel}</span>
			{#if currentModelObj?.provider}
				<span class="text-[9px] text-hecate-400">{currentModelObj.provider}</span>
			{/if}
		{:else}
			<span class="text-surface-600">·</span>
		...
		<button
			class="text-[10px] px-2 py-0.5 rounded cursor-pointer transition-colors
				{currentView === 'chat' ? 'text-hecate-400 bg-hecate-600/20' : 'text-surface-500 hover:text-surface-300'}"
			onclick={() => { currentView = 'chat'; tick().then(() => inputEl?.focus()); }}
		>chat</button>
		<button
			class="text-[10px] px-2 py-0.5 rounded cursor-pointer transition-colors
				{currentView === 'models' ? 'text-hecate-400 bg-hecate-600/20' : 'text-surface-500 hover:text-surface-300'}"
	...
	{#if currentView === 'models'}
			...
			{:else}
				{#each groupedModels as group}
					...
					{#each group.models as model}
						...
						>
							<span class="w-3 text-center shrink-0 text-[8px] {isSelected ? 'text-hecate-400' : 'text-surface-600'}">
								{isSelected ? '\u25CF' : ' '}
							</span>
							<span class="flex-1 truncate font-mono text-[10px] {isSelected ? 'text-hecate-400' : ''}">{model.name}</span>
		...
		<div class="border-t border-surface-700 bg-surface-800/80 px-3 py-1 shrink-0 flex items-center gap-2 text-[10px] min-h-[24px]">
			{#if modelMode === 'search'}
				<span class="text-hecate-400">/{modelSearch}<span class="animate-pulse">_</span></span>
	...
	{:else}
			...
			{#if $messages.length === 0 && !$isStreaming}
				<div class="flex items-center justify-center h-full">
					<div class="text-center text-surface-500 text-[11px] space-y-1">
						...
						{#if $selectedModel}
							<div>Using <span class="text-hecate-400">{$selectedModel}</span></div>
						{:else}
							<div>No model — <span class="text-hecate-400">Tab</span> to browse or <span class="text-hecate-400">:model &lt;name&gt;</span></div>
						{/if}
		...
		<div class="border-t border-surface-700 bg-surface-800/80 px-3 py-1.5 shrink-0">
			<div class="flex items-end gap-2">
				<span class="text-hecate-400 text-[11px] py-1.5 shrink-0">{$isStreaming ? '\u25CF' : '\u276F'}</span>
				<textarea
					bind:this={inputEl}
