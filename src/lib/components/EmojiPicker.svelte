...
>
	...
	<div class="flex gap-1 border-t border-surface-600 pt-2">
		<input
			...
			placeholder="Custom..."
			class="flex-1 bg-surface-800 text-surface-100 text-xs px-2 py-1 rounded outline-none border border-surface-600 focus:border-macula-500/50 placeholder:text-surface-500"
			onkeydown={(e) => {
				if (e.key === 'Enter') handleCustomSubmit();
			}}
		/>
		<button
			class="px-2 py-1 text-xs bg-macula-600 hover:bg-macula-500 text-white rounded cursor-pointer"
