<script lang="ts">
	import type { ChatMessage as ChatMsg } from '../types.js';
	...
	const parts = $derived.by((): ContentPart[] => {
		...
		while (remaining.length > 0) {
			...
			if (closeIdx === -1) {
				// Unclosed think block — treat rest as think
	...
	const hasThinking = $derived(parts.some(p => p.type === 'think'));
</script>
...
<div class="flex {isUser ? 'justify-end' : 'justify-start'} mb-3">
	<div
		class="max-w-[80%] rounded-lg px-4 py-2.5 text-sm leading-relaxed
			...
			: isError
				? 'bg-health-err/10 border border-health-err/30 text-health-err'
