<script lang="ts">
	...
	// While streaming, detect if we're inside a <think> block
	...
	// Split content: before </think> is reasoning, after is the actual response
	const visibleContent = $derived.by(() => {
		if (!hasCompletedThink) {
			// Still in think block or no think block at all
			if (isInThink) return '';
			return content;
		}
		// Think block is closed — show only the part after </think>
	...
	const thinkContent = $derived.by(() => {
		...
		if (closeIdx === -1) {
			// Still thinking — show what we have so far
			return content.slice(openIdx + 7);
		}
	...
	>
		...
		{:else if visibleContent}
			...
			<div class="whitespace-pre-wrap break-words">{visibleContent}<span
				...
				></span></div>
		{:else}
			<!-- No content yet (waiting for first token) -->
