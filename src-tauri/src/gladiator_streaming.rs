...
pub async fn gladiator_training_stream(
    ...
    let progress_event = format!("gladiator-progress-{}", stream_id);
    let done_event = format!("gladiator-done-{}", stream_id);
    let error_event = format!("gladiator-error-{}", stream_id);

    eprintln!(
        "[gladiator_training_stream] starting stream_id={} stable_id={}",
        stream_id, stable_id
    );

    std::thread::spawn(move || {
        let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            do_gladiator_stream(&app, &progress_event, &stable_id)
        }));

        match result {
            Ok(Ok(())) => {
                eprintln!("[gladiator_training_stream] completed normally");
                let _ = app.emit(&done_event, serde_json::json!({"type": "done"}));
            }
            Ok(Err(e)) => {
                eprintln!("[gladiator_training_stream] error: {}", e);
                let _ = app.emit(
                    &error_event,
            ...
            Err(panic_info) => {
                ...
                eprintln!("[gladiator_training_stream] panic: {}", msg);
                let _ = app.emit(
                    &error_event,
...
fn do_gladiator_stream(
    ...
    let http_req = format!(
        "GET /api/arcade/gladiators/stables/{}/stream HTTP/1.1\r\n\
         Host: localhost\r\n\
    ...
    eprintln!("[gladiator_training_stream] status: {}", status_code);
    ...
    if !is_chunked {
        eprintln!("[gladiator_training_stream] WARNING: chunked not detected, forcing chunked mode");
    }

    // Cowboy stream_reply is always chunked
    read_sse_chunked(app, progress_event, &mut reader)
}
...
fn read_sse_chunked(
    ...
    loop {
        ...
        while let Some(newline_pos) = leftover.find('\n') {
            ...
            process_sse_line(app, progress_event, &line);
        }
    }
...
fn process_sse_line(app: &AppHandle, progress_event: &str, line: &str) {
