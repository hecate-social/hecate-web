...
fn parse_installed_version(container_path: &PathBuf) -> Option<String> {
    ...
    for line in content.lines() {
        ...
        if trimmed.starts_with("Image=") {
            // Image=ghcr.io/hecate-social/hecate-marthad:0.1.1
...
pub async fn check_plugin_updates() -> Result<Vec<PluginUpdate>, String> {
    ...
    // Discover plugins from ~/.hecate/ (same logic as plugin_discovery)
    ...
    for plugin_name in &plugin_names {
        ...
        let container_file = apps_dir.join(format!("hecate-{}d.container", plugin_name));
        ...
        let url = format!(
            "https://api.github.com/repos/hecate-social/hecate-{}/releases/latest",
            plugin_name
        );
        ...
        if is_newer(&remote_version, &installed) {
            eprintln!(
                ...
                plugin_name, installed, remote_version
            );
...
pub async fn install_plugin_update(
    ...
    let apps_dir = gitops_apps_dir().ok_or("Cannot determine gitops apps directory")?;
    let container_file = apps_dir.join(format!("hecate-{}d.container", name));
    ...
    let updated: Vec<String> = content
        ...
        .map(|line| {
            let trimmed = line.trim();
            if trimmed.starts_with("Image=") && trimmed.contains(&format!("hecate-{}d:", name)) {
                found = true;
                new_image.clone()
            } else {
                line.to_string()
    ...
    if !found {
        return Err(format!("No Image= line found for hecate-{}d in .container file", name));
    }
    ...
    eprintln!("[plugin-updater] Restarting hecate-{}d...", name);
